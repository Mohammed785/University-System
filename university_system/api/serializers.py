...
class CourseSerializer(serializers.ModelSerializer):
    ...
    assignments = serializers.HyperlinkedRelatedField(
        view_name="assignment-detail", read_only=True, many=True, lookup_field="slug"
    )
    quizzes = serializers.HyperlinkedRelatedField(
        view_name="quiz-detail", read_only=True, many=True, lookup_field="slug"
    )
    announcements = serializers.HyperlinkedRelatedField(
        view_name="announcement-detail", read_only=True, many=True, lookup_field="slug"
    )

    class Meta:
        model = Course
        fields = [
            "name","course_code","description","professor","students","quizzes","assignments","announcements"
        ]
...
class QuizSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Quiz
        fields = [
            "name","start","duration","end_time","open","total_grade","attempts","course","prof","questions",
        ]
...
class AssignmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Assignment
        fields = ["file", "deadline", "max_mark", "notes", "upload_date", "professor", "course", "slug"]
...
class AnswersSerializer(serializers.ModelSerializer):
    date_posted = serializers.DateTimeField(format="%m/%d/%Y, %H:%M",read_only=True)
    author = serializers.StringRelatedField()
    question = serializers.StringRelatedField()

    class Meta:
        model = Answers
        fields = ["body", "edited", "date_posted", "author", "question",'slug']
...
class QuestionsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Questions
        fields = ["body", "edited", "date_posted", "course", "author", "answers",'slug']
