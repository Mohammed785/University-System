...
class UserAbstract(AbstractBaseUser, PermissionsMixin):
    ...
    email = models.EmailField(
        ...
        max_length=150,
        unique=True,
        error_messages={"unique": _("Email Is Already Exists")},
...
class User(UserAbstract):
    ...
    college_id = models.CharField(
        max_length=6, null=True, blank=True, unique=True, error_messages={"unique": "Please Enter Unique Email"}
    )
    personal_email = models.EmailField(
        ...
        null=True,
        unique=True,
        validators=[EmailValidator("Enter A valid Email", "invalid", ["gmail", "yahoo", "hotmail", "outlook"])],
        error_messages={"unique": "Please Enter Unique Email"},
    )
    ...
    def save(self, *args, **kwargs):
        if not self.college_id:
            self.college_id = ID_generator(self)
